...
- **Commonwealth Federation** (focus-union; AI: `base = 0`) — `05_united_kingdom.txt` capstones `ENG_the_crown_realms_united` / `ENG_the_dominions_restored` (set the cosmetic once `ENG_uq_canzuk_integrated`); annexation runs through `ENG_uq_enter_stage_4` in `99_ENG_scripted_effects..txt`; Ireland below. Writes: cosmetic `ENG_commonwealth_federation`; cores + annex via the ladder. Ratchet: sub-step of CANZUK (id 15).
